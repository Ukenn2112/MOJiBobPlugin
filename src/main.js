...
function translate(query, completion) {
  ...
  (async () => {
    ...
    if (search_resp.error) {
      ...
      $log.info(
        `搜索请求结果 search_data: ${JSON.stringify(search_resp.data)}`
      );
      const search_data = search_resp.data.result.results["search-all"].result;
      if (
        search_data.word === undefined ||
        search_data.word.searchResult.length === 0
      ) {
        completion({
          error: {
            type: "notFound",
      ...
      const to_dict = {};
      to_dict["word"] = `${world_data.word.spell} ${
        world_data.word.accent || ""
      }`;
      to_dict["parts"] = [];
      ...
      if (_world_data.thesaurus) {
        if (
          _world_data.thesaurus.synonyms &&
          _world_data.thesaurus.synonyms.length !== 0
        ) {
        ...
        if (
          _world_data.thesaurus.antonyms &&
          _world_data.thesaurus.antonyms.length !== 0
        ) {
          to_dict["exchanges"].push({
            name: "反义词",
          ...
          for (const subdetail of world_data.subdetails) {
            if (detail.objectId === subdetail.detailsId) {
              to_dict["additions"].push({
                name: ``,
                value: `${num}、[${detail.title.replace(/#/g, "·")}] ${
                  subdetail.title
                }`,
              });
              num += 1;
